...
export default function DashboardPage() {
  ...
  const progressToNext = nextTier
    ? ((currentPoints - loyaltyTiers[currentTierIndex].min) /
        (nextTier.min - loyaltyTiers[currentTierIndex].min)) *
      100
    : 100;
          ...
          {demoOrders.map((order, i) => (
            <Link
              ...
              className={cn(
                ...
                i < demoOrders.length - 1 && "border-b border-border"
              )}
            >
            ...
            <div>
              <p className="text-sm font-medium">
                ...
                <span className="text-gold-dark">
                  {loyaltyTiers[currentTierIndex].name}
                </span>
              </p>
              <p className="text-xs text-muted">
                {currentPoints} points •{" "}
                {nextTier
                  ? `${nextTier.min - currentPoints} points avant le niveau ${nextTier.name}`
          ...
          <div className="relative">
            <div className="flex justify-between text-[10px] text-muted mb-2">
              {loyaltyTiers.map((tier) => (
                <span
                  ...
                  className={cn(
                    "font-medium",
                    currentPoints >= tier.min && "text-gold-dark"
                  )}
                >
                  {tier.name}
                </span>
            ...
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={{
                  width: `${((currentTierIndex + progressToNext / 100) / (loyaltyTiers.length - 1)) * 100}%`,
                }}
