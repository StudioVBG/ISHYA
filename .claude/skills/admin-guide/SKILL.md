...
> Note : les tables `admin_users` / `admin_roles` ont été supprimées (migration 008) car elles n'étaient jamais branchées au code. Si on doit reintroduire un système de permissions granulaires (super_admin, support, editor), partir d'une nouvelle migration.
